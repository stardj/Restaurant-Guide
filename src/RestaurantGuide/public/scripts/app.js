 ...
 */
function initRestaurantPWA() {
    loadData("index", new Date().getTime());
 ...
 * @param date
 */
function loadData(name, date){
    getCachedData(name, date);
    const input = JSON.stringify({name: name, date: date});
    $.ajax({
        ...
        error: function (xhr, status, error) {
            showOfflineWarning();
            //addToResults(getCachedData(name', date));
